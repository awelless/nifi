...
public class ConsumeKinesisV2 extends AbstractProcessor {
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        maybeLease.ifPresentOrElse(
                lease -> processRecordsFromBuffer(context, session, lease),
                context::yield
        );
    }

    private void processRecordsFromBuffer(final ProcessContext context, final ProcessSession session, final ShardBufferLease lease) {
        try {
            ...
            if (readerFactory == null) {
                processRecordsAsRaw(session, records, lease.shardId());
            } else {
                final RecordSetWriterFactory writerFactory = context.getProperty(RECORD_WRITER).asControllerService(RecordSetWriterFactory.class);
                processRecordsWithReader(session, records, lease.shardId(), readerFactory, writerFactory);
            }
    ...
    private void processRecordsAsRaw(final ProcessSession session, final List<KinesisClientRecord> records, final String shardId) {
        for (final KinesisClientRecord record : records) {
            final FlowFile flowFile = session.create();

            session.write(flowFile, out -> Channels.newChannel(out).write(record.data()));

            addKinesisAttributes(flowFile, session, List.of(record), shardId);
    ...
    private void processRecordsWithReader(final ProcessSession session, final List<KinesisClientRecord> records, 
                                        final String shardId, final RecordReaderFactory readerFactory,
                                        final RecordSetWriterFactory writerFactory) {
        // todo
    }

    private void handleParseFailure(final ProcessSession session, final KinesisClientRecord record, 
                                  final String shardId, final Exception e) {
        FlowFile flowFile = session.create();
        
        flowFile = session.write(flowFile, out -> {
            final ByteBuffer dataBuffer = record.data();
        ...
        addKinesisAttributes(flowFile, session, List.of(record), shardId);
        flowFile = session.putAttribute(flowFile, "parse.failure.reason", e.getMessage());
        session.transfer(flowFile, REL_PARSE_FAILURE);
    }

    private void addKinesisAttributes(final FlowFile flowFile, final ProcessSession session,
                                        final List<KinesisClientRecord> records, final String shardId ) {
        ...
        if (records.size() == 1) {
            final KinesisClientRecord record = records.getFirst();
            attributes.put("aws.kinesis.partition.key", record.partitionKey());
            attributes.put("aws.kinesis.sequence.number", record.sequenceNumber());
        } else {
            // For multiple records, add range information
            attributes.put("aws.kinesis.record.count", String.valueOf(records.size()));
            if (!records.isEmpty()) {
                attributes.put("aws.kinesis.sequence.number.first", records.getFirst().sequenceNumber());
                attributes.put("aws.kinesis.sequence.number.last", records.getLast().sequenceNumber());
            }
        }
